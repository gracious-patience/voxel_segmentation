...
from src.datasets import ImageLabelDataset, FeatureDataset
from src.feature_extractor import create_feature_extractor, collect_features
...
def prepare_data(args):
    ...
    X = torch.zeros((len(dataset),args['num_of_features'] , *args['shape'][1:]), dtype=torch.float32)
    y = torch.zeros((len(dataset), *args['shape'][1:]), dtype=torch.float32)
...
def evaluation(args, models):
    ...
    for img, label in tqdm(dataset):        
        ...
        x = features.view(args['num_of_features'], -1).permute(1, 0)
        pred, uncertainty_score = predict_labels(
            models, x, size=args['shape'][1:]
        )
        gts.append(label.numpy().argmax(0))
        preds.append((pred.to(args['device'])*img[0][0]).cpu().numpy())
        # preds[-1] = (pred*img[0][0]).numpy()
...
def train(args):
    ...
    for MODEL_NUMBER in range(args['start_model_num'], args['model_num'], 1):
        ...
        gc.collect()
        classifier = pixel_classifier(numpy_class=(args['number_class']), dim=args['num_of_features'])
            ...
            for X_batch, y_batch in train_loader:
                ...
                if epoch > 3:
                    ...
                    if break_count > 50:
                        ...
                        break
            if epoch % args['save_period'] == 0 and args['save_intermediate']:
                model_path = os.path.join(args['exp_dir'], 
                                  'model_' + str(MODEL_NUMBER) + '_epoch' + str(epoch) + '.pth')
