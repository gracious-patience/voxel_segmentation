...
class Experiment(DiffusionExperiment):
    ...
    def sample_fn(self, times):
        print("here")
        for x in self.train_loader:
            print("here")
            shape = x[0].shape
            break
        torch.save(self.model.p_sample_loop(shape), "/home/sharfikeg/my_files/multinomial_diffusion/logs/"+ self.args.name + ".pt")
        print("here")
    ...
    def eval_fn(self, epoch):
        ...
        with torch.no_grad():
            ...
            loss_count = 0
            for x in self.train_loader:
                loss = elbo_bpd(self.model, x[0].to(self.args.device))
                loss_sum += loss.detach().cpu().item() * len(x)
                loss_count += len(x)
